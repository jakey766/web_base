...
$.sysop={
	...
	popup:{
		...
		confirm:function(tit,cont,css,callback,isCache,callback2){//弹出确认框，参数css可省略设置
			...
			if(!$('#popConfirm').length){
				...
				$('#popConfirm').html('<dl><dt>'+tit+'<a class="close" href="#popConfirm"></a></dt><dd><div class="cont"><div class="cont_t">'+cont+'</div><div class="btn"><input type="button" alt="#popConfirm" class="ok close button" value="确认" /> <input type="button" alt="#popConfirm" class="close button" value="取消" /></div></div></dd></dl>')
		...
		confirm_new:function(tit,cont,css,callback,isCache,callback2){//弹出确认框，参数css可省略设置
			...
			if(!$('#popConfirm').length){
				$('body').append('<div id="popConfirm" class="pop-cont popConfirm"></div>');
				$('#popConfirm').html('<dl><dt>'+tit+'<a class="close" href="#popConfirm"></a></dt><dd><div class="cont"><div class="cont_t">'+cont+'</div><div class="btn"><input type="button" alt="#popConfirm" class="ok button" value="确认" /> <input type="button" alt="#popConfirm" class="close button" value="取消" /></div></div></dd></dl>')
				.find('.ok').click(function(){callback&&callback.call($('#popConfirm')[0])}).end().find('.close').click(function(){callback2&&callback2();});
			}
			if(css) $('#popConfirm').css(css);
			this.box('#popConfirm');
	...
	pagination:{
		toPage:function(n,self){
			...
			//console.log("self.total is: "+self.total);
			if(num/1>self.total/1||num/1<1||isNaN(num/1)) {
				alert('不存在此页面！');
				return;
				}
	...
	autoComplete:{
		...
		selectBox:function(select,param){//初始化selectBox
			...
			var handlerData=function(data,name){//处理数据
				...
				textKey.css({width:sWidth-30,height:sHeight-2,lineHeight:sHeight-2+'px'});//设置input样式
			...
			input.keyup(function(e){
				if(e.keyCode == '38'||e.keyCode == '40'||e.keyCode == '13') {
					...
					if(e.keyCode == '13') {//Enter回车
						...
						} else input.parent().nextAll(':button:eq(0)').click();//搜索
					}
				} else{
					var val = input.val();
					...
					if(!data){//正常处理
						$.ajax({
							...
							success:function(data){
								...
								if(data.success) json = data.object;//兼容新格式
	...
	validation:{
		addMsg:function(str,input){//添加信息
			this.obj=$(input)||this.obj;//让input可自定义，这样可解决ajax的延迟导致tipPar判断错误问题
			this.tipPar=$(input)?$(input).closest('td'):this.tipPar;
			if(str===true){//正确
				...
				var err=this.tipPar.find('.error');
				if(err.length){
					if(err.html().indexOf(str)==-1) err.find('.tips').append('<li>✗ '+str+'</li>');
					else return;
				}
				else this.tipPar.append('<div class="inputTips error"><ul class="tips"><li>✗ '+str+'</li></ul></div>');
			}
		},
		delMsg:function(input,isFromPar){//删除信息
			this.saveInput(input);//保存
			var $obj=this.obj,$par=this.tipPar;
			if(isFromPar) $par.find('.errorStatus').removeClass('errorStatus');//Par下的全部删除
			else $obj.removeClass('errorStatus');//只删除自己
			$par.find('.inputTips').remove();//提示信息删除
		},
		...
		inputCheck:function(self){//===========验证1（INPUT & TEXTAREA）
			...
			if($(this).is('.word')){//单词
				if(this.value&&!/^[\w,.,-]+$/.test(this.value)) self.addMsg('请输入英文/数字/下划线');
			}
		...
		saveInput:function(input){//保存input和它的parent
			this.obj=$(input);//保存1
			this.tipPar=$(input).closest('td').find('.item').length?$(input).closest('.item'):$(input).closest('td');//保存2(默认tips位置)
		},
		valid:function(self,isSubmit){//触发验证方法，参数isLimit保证只有check事件时才触发myCheck(可选择)
			self.saveInput(this);//保存
			if($(this).is(':text,textarea')) self.inputCheck.call(this,self);
			if($(this).is('select')) self.selectCheck.call(this,self);
			if(isSubmit) $(this).trigger('mySubmit');//提交的自定义验证
		},
		check:function(form){//手动验证表单（submit提交）
			var $form=form?$(form):$(this.form);
			this.reqFilter='td:has(.valItem)';//必填项过滤器
			$form.find(':text,select,textarea').filter(':visible:not(:disabled)').trigger('checkSubmit');
			$form.find('.myCheck').filter(':visible:not(:disabled)').trigger('checkSubmit');//最后验证myCheck（可以省略？？）
			if(n=$form.find('.errorStatus:visible').length) {//把.error改为.errorStatus:visible @9.26
		...
		init:function(form,hidCorTip,fn){//验证表单初始化
			...
			$form.addClass('validForm').find('.required').closest('td').prev().prepend('<b class="starQR">*</b>');//格式化required样式
			$form.find('td').bind({//绑定td保证验证整行:input的同一时间触发
				'focusout':function(){$(this).find(':input:visible:enabled').trigger('check');}
			}).find(':input').bind({//验证事件
	...
	kit:{
		browserCheck:function(){//浏览器检测
		    ...
		    if(ua.indexOf('chrome')!=-1){
		    	...
		    	$('body').empty().prepend('<div id="IEError" style="margin-top:-10px;font-size:16px;line-height:30px;width:100%;text-align:center;background:#f1f1f1;"><p style="margin:100px 0 20px;"><p><b>提示：</b>本系统应用了最新WEB技术(HTML5及CSS3)，为了正常使用本系统，请使用 <b>谷歌浏览器</b>(版本10或以上)。</p><p><a href="http://www.google.com/chrome?hl=zh-cn">点此下载谷歌浏览器</a></p></div>');
			}
		},
		addLineItem:function(btn,callback){//增加一行ITEM
			var $btn=$(btn),str=$btn.attr('alt'),copy=$btn.parent().parent().find('.copyPrev').eq(0);
			if(str) var line=$(str.substring(str.indexOf('#')));
			else if(copy.length) var line=copy;
			else var line=$btn.parent();
			line.parent().append(line.clone(true).removeAttr('id').addClass('item').css('display','block').find('input[type="button"]:disabled').attr('disabled',false).end());
			callback&&callback();
		},
		delLineItem:function(btn,callback){//删除一行ITEM
			$(btn).parent().remove();
			callback&&callback();
		},
		...
		selectChange:function(selectObj,changesObj,string,option,isAll){//下拉select,默认changesObj即为它的next select
			var $selectObj=$(selectObj);
			if($(changesObj).length) $changesObj=$(changesObj);
			else{
				$changesObj=$selectObj.nextAll('select').eq(0);
				isAll=option,option=string,string=changesObj;
			}
			(isAll?$selectObj.nextAll('select'):$changesObj).html("<option value='-1'>请选择</option>");
			if($selectObj.val()=='0') return;//当为0值退出
			$.getJSON(string+$selectObj.val(),function(data){
				$.each(data,function(){
					$changesObj.append("<option value="+this[option&&option.value||'id']+">"+this[option&&option.name||'name']+"</option>");
				});
			});
		},
		...
		initFormByJSON:function(box,obj,keepInput,fn){//keepInput参数可省略
			...
			if(keepInput!==true){
				...
				$.each(obj,function(i,n){
					...
					/*if($i.length>1){//如果是radio/checkbox(只支持单选)
						if($i.is(':input[type=checkbox')){//checkbox
							alert($i.length)
							$.each(n,function(){
								$i.filter('[value="'+this+'"]').attr('checked',true);
							});
					...
					}*/ else if( $i.is('select')){ //select
						var sl_option = $i.find('option').filter('[value="'+n+'"]');
						sl_option.attr('selected',true);
						if( $i.prev().is('.selectBoxInput')){
							$i.prev().val(sl_option.text());
						}
					} else{
						$i.val(n);
					}
		...
		initInputByParam: function(box, param) {
			var ps = param.split('&');
			$(box).find(':input[name]').each(function() {
				...
				for(var i = 0; i < ps.length; i++) {
					...
					if(thiz.name == p[0]) {
						thiz.value = p[1];
						if(thiz.type == 'checkbox') thiz.checked = true;
					}
				}
			});
			var json = this.deParam(param);
			return this.str2Obj(json);
		},
