...
$.tools.commonPage = function(box, fn, disR, count, p, pCount) {
	...
	var pagination = $.metrics.getPagination();
...
$.tools.multiSelect = {
	...
	init: function(id, value) {
		...
		var thiz = this;
		$.getJSON('/common/listAppUser.do', function(data) {
	...
	initMultiSelect: function(id, value) {
		...
		$.each(this.list, function(i, item) {
			if (!item.user_name || item.user_name == '') return true;
			var val = value && value.length > 0 ? value.split(',') : [];
			var s = '';
			for(var j in val) {
				if (item.user_name == val[j] || item.nick_name == val[j]) { //2013-02-26 增加 中文名
					s = ' selected';
					notSelected = 1;
					break;
				}
			...
			html += '<option value="' + item.user_name + '"'+ s + '>' + item.nick_name +'</option>';
		});
		$('#' + id).html(html);
...
$.tools.singleSelect = {
		...
		init: function(id, value) {
			if (this.list && this.list.length > 0) {
				this.initSelect(id, value);
				return;
			}
			var thiz = this;
			$.getJSON('/common/listAppUser.do', function(data) {
				if(thiz.list && thiz.list.length > 0) { // 最后去重检测：由于异步特性，在加载完毕后，list数据有可能被外部更改
					thiz.initSelect(id, value);
					return;
				}
				var user = data.object;
				for(var i in user) {
		...
		initSelect: function(id, value) {
			...
			$.each(this.list, function(i, item) {
				...
				html += '<option value="' + item.user_name + '"'+ s + '>' + item.nick_name+' '+item.user_name +'</option>';
			});
			$('#' + id).html(html);
			$('#' + id + '.chzn-select').trigger("liszt:updated"); // 2012-11-08
			$('#' + id + '.chzn-select').chosen();
