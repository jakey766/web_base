 ...
 * 增加 点击箭头 修改时间，清空按钮改为确认按钮。zenglican 2013-12-03
 * 如果限制了日期的最大值，最小值，将不能调整时间 2014-08-06 10:40:45
 */
...
calendarTpl =
'<table class="lcui_border" border="0" cellspacing="1" cellpadding="0">' +
'<tbody>' +
'<tr>' +
	'<td class="lcui_lt"></td>' +
	'<td class="lcui_t"></td>' +
	...
	'<td>' +
		...
		'<div class="cui_foot">' +
			...
			'<tr>' +
				...
				'<td align="center" class="lcui_time clearfix"><div class="time-change"><span title="增加" class="time-change-trigger time-change-up">▲</span><input min="0" max="24" class="cui_hour" maxlength="2"/><span title="减少" class="time-change-trigger time-change-down">▼</span></div><div class="time-change"><span title="增加" class="time-change-trigger time-change-up">▲</span><input class="cui_minute" min="0" max="60" maxlength="2"/><span title="减少" class="time-change-trigger time-change-down">▼</span></div><div class="time-change"><span title="增加" class="time-change-trigger time-change-up">▲</span><input class="cui_second" min="0" max="60" maxlength="2"/><span title="减少" class="time-change-trigger time-change-down">▼</span></div></td>' +
				'<td align="center" class="lcui_empty"><a class="cui_okbtn" href="javascript:void(0);">确定</a></td>' +
			'</tr>' +
			'</table>' +
		'</div>' +
	'</td>' +
...
lhgcalendar.fn = lhgcalendar.prototype =
{
	...
	// 增加时间可点击增加、减少功能 by zenglican 2013-12-02
	_setTime: function(){
		var config = this.config;
		if(config.maxDate || config.minDate){
			$('.time-change input').attr('disabled',true);
			return;
		}
		var numCtn = $(".time-change");
		numCtn.find(".time-change-trigger").off('click');
		numCtn.find(".time-change-up").on('click', '', function() {
			var input = $(this).parent().find("input"), type = input.attr('class');
			if(type === 'cui_hour'){
		...
		numCtn.find(".time-change-down").on('click', '', function(event) {
			var input = $(this).parent().find("input"), type = input.attr('class');
			if(type === 'cui_hour'){
	...
	_addEvent: function()
	{
		...
		DOM.wrap.bind('click',function(evt){
			...
			else if( target === DOM.okbtn[0]){ // add by zenglican 2013-12-03
				var today = new Date();
				that.year = today.getFullYear();
				that.month = today.getMonth() + 1;
				that.day = today.getDate();
